...
import 'package:flutpuz_hack/position/starting_position.dart';
...
abstract class Game {
  ...
  Iterable<Tile> findChips(Board board, {@required Point<int> point});
}
...
class _GameImpl implements Game {
  ...
  Board hardest(Board board) {
    ...
    // Chips
    final chips = List.of(board.tiles, growable: false);
  ...
  Board shuffle(Board board, {int amount = 300}) {
    ...
    List<List<Tile>> matrix = List.generate(board.size, (i) {
      return List.generate(board.size, (j) {
        return null;
      });
    });

    board.tiles.forEach((chip) {
    ...
    for (var n = 0; n < amount; n++) {
      ...
      switch (random.nextInt(4)) {
        ...
        case 3: // left
          ...
          break;
        default:
          throw StateError("You have choosen an unknown direction.");
          break;
      }
  ...
  @override
  Iterable<Tile> findChips(Board board, {Point<int> point}) {
    if (point.x == board.blank.x) {
      int start;
      int end;
